...
func Initialize(source *settings.Source, mock bool) {
	...
	if !newIndex.Config.DisableIndexing {
		slog.Info("initializing index: [%v]", newIndex.Name)
		...
		slog.Debug("indexing disabled for source: " + newIndex.Name)
	}
}
		...
		if modChange {
			...
			for _, item := range cacheDirItems {
				...
				if err != nil && err != errors.ErrNotIndexed {
					slog.Error("error indexing directory %v : %v", combinedPath+item.Name, err)
				}
			}
			return nil
		}
	...
	for _, file := range files {
		...
		if isDir {
			...
			if config.Recursive {
				...
				if err != nil {
					slog.Error("Failed to index directory %s: %v", dirPath, err)
					continue
				}
			}
			realDirInfo, exists := idx.GetMetadataInfo(dirPath, true)
